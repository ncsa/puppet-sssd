...
describe 'sssd' do
  ...
  describe file('/etc/sssd/sssd.conf') do
    ...
    it { is_expected.to be_owned_by 'root' }
    it { is_expected.to be_grouped_into 'root' }
    its(:content) { is_expected.to match %r{^ \[ sssd \] $}x }
    its(:content) { is_expected.to match %r{^ domains = example\.com $}x }
    its(:content) { is_expected.to match %r{^ services = nss $}x } if fact('operatingsystemmajrelease').eql?('6')
    its(:content) { is_expected.to match %r{^ services = $}x } unless fact('operatingsystemmajrelease').eql?('6')
    its(:content) { is_expected.to match %r{^ \[ domain/example\.com \] $}x }
  end
