...
define sssd::domain (
  ...
  Optional[String]                                                         $dyndns_iface                                  = undef,
  ...
  # Create a datacat_collector for each array_type parameter
  $array_parameters = {
  ...
  $array_parameters.each |$param, $rawval| {
    datacat_collector { "sssd/domain/${domain}/${param}" :
      target_resource => Sssd_conf["domain/${domain}/${param}"],
      before          => Sssd_conf["domain/${domain}/${param}"],
      target_field    => 'value',
      template_body   => '<%= @data["items"] and @data["items"].uniq.sort.join(",") %>',
    }

    if $rawval =~ NotUndef {
      ::sssd::domain::append_array {
        "sssd initial array value for parameter '${param}' in domain '${domain}'" :
          domain  => $domain,
          setting => $param,
          items   => $rawval,
      }
